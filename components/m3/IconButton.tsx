...
export function M3IconButton({ icon, onClick, selected, tooltip, size = 40, variant = "standard", filled = false, ...rest }: Props) {
  const [hover, setHover] = React.useState(false);
  const [press, setPress] = React.useState(false);

  const palettes: Record<IconButtonVariant, { bg: string; fg: string; selectedBg: string; selectedFg: string }> = {
