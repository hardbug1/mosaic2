...
const STEPS = [
  {
    ...
    color: "#6750A4",
    title: "첫 게시물 추가하기",
    desc: "메모, 이미지, 링크, 영상, 파일 무엇이든 올릴 수 있어요.",
    action: "addPost" as const,
  },
  {
    icon: "group_add",
    color: "#00639B",
    title: "팀원 초대하기",
    desc: "이메일이나 링크로 동료를 불러 함께 채워보세요.",
  ...
  },
  {
    ...
    color: "#386A20",
    title: "섹션으로 정리하기",
    desc: "컬럼·그리드·캔버스 레이아웃으로 자유롭게 배치해요.",
...
export function EmptyBoard({ onAddPost, onInvite }: Props) {
  const [done, setDone] = useState<Record<number, boolean>>({});

  const toggleDone = (i: number) =>
    setDone((d) => ({ ...d, [i]: !d[i] }));

  const handleStepClick = (i: number, action: "addPost" | "invite" | "section") => {
    toggleDone(i);
    if (action === "addPost") {
      onAddPost();
    ...
    // "section" is a visual hint — no action yet
  };
      ...
      >
        ...
        >
          ...
          <div style={{ textAlign: "center" }}>
            ...
            >
              작은 생각 하나가 모자이크의 시작이에요. 아래에서 첫 게시물을
              추가해보세요.
            </div>
          </div>
          ...
          <M3Button
            ...
            icon="add"
            onClick={onAddPost}
            style={{ height: 52, borderRadius: 9999, padding: "0 28px" }}
          >
            첫 게시물 추가하기
          </M3Button>
            ...
            {STEPS.map((s, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`md-icon${isDone ? " is-filled" : ""}`}
                    style={{
                      fontSize: 24,
                      color: isDone ? "#386A20" : "var(--md-sys-color-outline)",
                    }}
                  >
