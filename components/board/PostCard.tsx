...
function VideoBlock({ dur, url }: { dur?: string | null; url?: string | null }) {
  const inner = (
    <div
      style={{
        height: 132,
  ...
  if (url) {
    return (
      <a href={url} target="_blank" rel="noreferrer" style={{ display: "block", textDecoration: "none" }} onClick={(e) => e.stopPropagation()}>
        {inner}
      </a>
    );
  }
...
function LinkPreview({ title, domain }: { title?: string | null; domain?: string | null }) {
  return (
    <div
      style={{
        display: "flex",
...
function LikeButton({
  ...
  return (
    ...
    >
      <span
        className={`md-icon ${liked ? "is-filled" : ""}`}
        style={{
          fontSize: 18,
          transform: hover && !liked ? "scale(1.12)" : "scale(1)",
          transition:
...
export default function PostCard({
  ...
  // Derive domain from url if domain is null (for link posts)
  const displayDomain =
    post.domain ??
    (() => {
  ...
  return (
    ...
    >
      ...
      {post.text && (
        <div
          className={dense ? "md-body-small" : "md-body-medium"}
          style={{
            color: "var(--md-sys-color-on-surface)",
            textWrap: "pretty",
            display: "-webkit-box",
            WebkitLineClamp: dense ? 4 : 8,
      ...
      {post.type === "link" && (
        <LinkPreview title={post.title} domain={displayDomain} />
      )}
      {post.type === "file" && (
        <FileChip
          title={post.fileName}
