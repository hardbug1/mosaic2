// Button.jsx — M3 buttons (filled, tonal, elevated, outlined, text, danger)
function M3Button({ variant = "filled", size = "md", icon, children, onClick, disabled, fullWidth, ...rest }) {
  const [hover, setHover] = React.useState(false);
  const [press, setPress] = React.useState(false);

  const HEIGHT = { sm: 32, md: 40, lg: 56 }[size];
  const PAD    = { sm: "0 16px", md: "0 24px", lg: "0 24px" }[size];
  ...
  return (
    <button {...rest}
      ...
      style={{
        ...
        height: HEIGHT,
        padding: variant === "text" ? "0 12px" : PAD,
        borderRadius: 9999,
